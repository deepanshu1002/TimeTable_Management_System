...
//				.collect(Collectors.toList());
		return classList.stream().map(classroom -> {
			ClassRoomDTO dto = mapper.map(classroom, ClassRoomDTO.class);
			dto.setDeptId(classroom.getDept().getDeptId());
			return dto;// Set deptId from Department
		}).collect(Collectors.toList());
	}
	...
	public ApiResponseDto addClassRoom(ClassRoomDTO addClassroomDTO) {
		...
		classRoom.setDept(dept);
		dept.setClassroom(classRoom);
