...
public class TimeTableMetadataController {
	...
	@GetMapping("/getnoofweeklyhrs/{dateString}/{deptId}")
	public ResponseEntity<?> getNoOfWeeklyHrs(@PathVariable String dateString,@PathVariable Long deptId){
		return ResponseEntity.status(HttpStatus.OK).body(dataService.getNoOfHrsWeekly(dateString,deptId));
	}
}
